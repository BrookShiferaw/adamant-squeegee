 #To do - Examine pupil dilation recording? 
...
#            stim_p_port.setData(int(0))        
        ...
        if t >= 0 and fb_team.status == NOT_STARTED:
            ...
            #Starts recording, sends team name (using fb_team tag) to eyetracker,
            #and in theory sends name of currently presented image.             
            #tracker.startRecording(1,1,1,1)  
            tracker.sendMessage("TRIALID " + str(fb_team.text))
            tracker.sendMessage("Current image = " + fb_images.image)
            tracker.sendMessage("!V IMGLOAD FILL " + fb_images.image)
